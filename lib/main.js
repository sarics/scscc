...
var btnIcons = {
	on: {
		...
		"48": data.url("icon48.png")
	},
	off: {
		"16": data.url("icon16_off.png"),
		"32": data.url("icon32_off.png"),
		"48": data.url("icon48_off.png")
	}
};
...
var panel = panels.Panel({
	...
	onShow: function() {
		this.port.emit("showPanel");
	},
	onHide: function() {
		if (sysver >= 29 && button.state("window").checked) {
			if (sysver >= 30) button.state("window", { checked: false });
			else timers.setTimeout(function() { button.state("window", { checked: false }); }, 300);
		}
	}
});
...
function panelFirstRun() {
	panel.port.emit("firstRun", [ ss.currRates, ss.currRatesLT, sp.prefs.btn ]);
...
panel.port.on("options", function() {
	window_utils.getMostRecentBrowserWindow().BrowserOpenAddonsMgr("addons://detail/" + encodeURIComponent(require('sdk/self').id) + "/preferences");
	panel.hide();
});
...
require("sdk/page-mod").PageMod({
	...
	contentScriptFile: data.url("scscc.js"),
			...
			if (sp.prefs.noti) {
				...
				if (ss.currRates.hasOwnProperty(fromCurr + "to" + toCurr)) {
					noti.notify({
						...
						text: fromCurr + " -> " + toCurr + " exchange rate updated:\n" 
